% FDTD 3D in one layer non dispersive skin with CPML
% Author:shayan dodge
...
%% FDTD variables
number_of_cells_per_wavelength=200;
...
boundary.air_buffer_number_of_cells_zn=300;
...
material_type(2).color=[0.98 0.92 0.84];
...
%% update source
Ex(1:nx,1:ny,n_cpml_zn+100)=Ex(1:nx,1:ny,n_cpml_zn+100)+muSource *(number * dt - t0)...
...
caxis([-2.5*10^15 2.5*10^15])
xlabel('x','fontsize', 12);ylabel('y','fontsize', 12);zlabel('z','fontsize', 12);
title({'Ex'}); 
drawnow
savefig(Hh_1,'one_layer_non_dispersive_skin_fdtd_cpml_gaussian_3D_3D.fig')
...
ylim([-2.5*10^15 2.5*10^15]);
grid on
savefig(Hh_2,'one_layer_non_dispersive_skin_fdtd_cpml_gaussian_3D_2D.fig')
end
disp(number)
end