...
%% update source
 Ex(1:nx,1:ny,n_cpml_zn+50)=Ex(1:nx,1:ny,n_cpml_zn+50)-amptidute *sin((number * dt - t0) * omega)/dz; % Sine source
...
title({'Ex'}); 
drawnow
savefig(Hh_1,'one_layer_non_dispersive_skin_fdtd_cpml_sinesoidal_3D_3D.fig')
...
ylim([-1*10^6 1*10^6]);
grid on
savefig(Hh_2,'one_layer_non_dispersive_skin_fdtd_cpml_sinesoidal_3D_2D.fig')
end
disp(number)
end