...
colormap(mymap)
colorbar
[x,y,z]=meshgrid(xcoor,ycoor, zcoor(1:nz));
h=slice(x,y,z,Ez_sample,0,[],[])
set(h, 'edgecolor', 'none')
%caxis([-1.5*10^6 1.5*10^6])
xlabel('x','fontsize', 12);ylabel('y','fontsize', 12);zlabel('z','fontsize', 12);
title({'Ex'}); 
drawnow
savefig(Hh_1,'three_layer_non_dispersive_skin_fdtd_cpml_sinesoidal_3D_3D.fig')
...
grid on
savefig(Hh_2,'three_layer_non_dispersive_skin_fdtd_cpml_sinesoidal_3D_2D.fig')
end
disp(number)
end